  ...
  return (
    <mesh ref={meshRef} position={[0, -2, 0]} rotation={[-Math.PI / 2, 0, 0]}>
      ...
      <meshStandardMaterial 
        ...
        emissiveIntensity={isHighLoad ? 0.5 : 1}
      />
    </mesh>
  );
};
...
const DataParticles = () => {
  const state = useHUD();
  const particleCount = useMemo(() => Math.min(Object.keys(state.events).length * 3 + 20, 80), [state.events]);
...
export const AmbientCanvas: React.FC = () => {
  ...
  const [swWebGL] = useState(() => hasWebgl && isSoftwareWebGL());
  ...
  if (!hasWebgl || swWebGL || isWebgpuActive || isMobile) {
    return (
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-0">
      ...
      >
        ...
        <DataParticles />
        <Stars radius={100} depth={50} count={isLight ? 200 : 500} factor={4} saturation={0} speed={1} />
      </Canvas>
    </div>
  );
};
