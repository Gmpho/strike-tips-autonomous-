...
The HUD SHALL flip no lock, armed, or saved state until the write response verifies (`res.ok`) and its body confirms the requested state: the emergency-stop control requires the response body's `status` field to match the requested state (`locked` for kill, `active` for reset), and settings writes require a success body. Any failed or unconfirmed write SHALL render an error state instead of an optimistic success.
