...
### Requirement: Pasted-card passthrough

The system SHALL NOT gate analysis of user-provided cards.

#### Scenario: User-pasted card is analysed
...
#### Scenario: Model-bound messages carry live grounding

- **WHEN** a message is bound for a model
- **THEN** the prompt SHALL carry today's SAST date, the live meetings
  list, and the no-invention rule, and the model SHALL be instructed to
  name missing cards plainly rather than invent them.
