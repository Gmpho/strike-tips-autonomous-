...
#### Scenario: TTS endpoint accepts a synthesis request

- **WHEN** a client POSTs a JSON payload containing `text` to `/api/tts`
- **THEN** the server SHALL return synthesized audio, defaulting `provider` to `"gemini"` when it is omitted.
...
#### Scenario: Gemini returns playable WAV audio

- **WHEN** `provider` is `"gemini"` and the model returns 24kHz linear PCM
- **THEN** the server SHALL convert it to RIFF WAV with a valid 44-byte header before responding.
...
#### Scenario: Long text is chunked and stitched gaplessly

- **WHEN** `provider` is `"groq"` and the input exceeds the 200-character model limit
- **THEN** the server SHALL split the text into sentences, synthesize each chunk, and stitch the WAV chunks into one gapless stream.

#### Scenario: Missing Groq key degrades to Gemini

- **WHEN** `provider` is `"groq"` but `GROQ_API_KEY` is not configured
- **THEN** the server SHALL return an informative error and offer or fall back to Gemini TTS.
...
#### Scenario: Voice studio plays and downloads audio

- **WHEN** a user converts text in the Text to Speech view
- **THEN** playback controls (play, pause, seek, speed 0.75x–1.5x) and WAV download SHALL be available for the synthesized audio.
