...
class HumanBehaviorSimulator:
    ...
    async def random_delay(self, min_ms: int = 1000, max_ms: int = 3000):
        ...
        delay = random.triangular(min_ms, max_ms, min_ms + (max_ms - min_ms) * 0.3) / 1000
        await asyncio.sleep(delay)
        self.actions += 1

    def _generate_bezier_path(self, start: MousePosition, end: MousePosition, steps: int = 20) -> List[MousePosition]:
        ...
        for i in range(steps + 1):
            ...
            path.append(MousePosition(int(x + random.randint(-2, 2)), int(y + random.randint(-2, 2))))
        return path

    async def move_mouse_naturally(self, page, x: int, y: int):
        """Move mouse in a curved path like a real hand."""
        target = MousePosition(x, y)
        path = self._generate_bezier_path(self.current_pos, target, steps=random.randint(15, 30))
        for point in path:
    ...
    async def type_naturally(self, page, selector: str, text: str):
        ...
        for char in text:
            ...
            if char == " ": delay *= 2
            if random.random() < 0.05: await asyncio.sleep(random.uniform(0.4, 0.9))
