...
class IntelligenceCacheManager:
        ...
        if self.snapshot_path.exists():
            try:
                with open(self.snapshot_path, 'r') as f:
                    ...
                    logger.info(f"💾 Intelligence Cache: Rehydrated {len(self.memory_state)} events from disk.")
    ...
    def update_baseline(self, event_id: str, runners: list):
        ...
        try:
            ...
            logger.error(f"❌ Intelligence Cache: History sync failed for {event_id}: {e}")
    ...
    def prune_stale_data(self, active_event_ids: list):
        ...
        try:
            ...
            if pruned_count > 0:
                logger.info(f"🧹 Intelligence Cache: Pruned {pruned_count} finished races from disk.")
        except Exception as e:
