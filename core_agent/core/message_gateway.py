...
class MessageGateway:
    ...
    def check_rate_limit(self, channel: Channel, user_id: str) -> tuple[bool, str]:
        ...
        return (
            False,
            f"[ERR] Rate limit exceeded. Try again in {int(60 / bucket.refill_rate)} seconds",
        )
        ...
        if session_key not in self._sessions:
            self._sessions[session_key] = {
                "session_id": hashlib.md5(
                    f"{session_key}:{time.time()}".encode()
                ).hexdigest()[:16],
                "channel": channel.value,
                "user_id": user_id,
    ...
    def _get_security_profile(self, profile_id: str) -> SecurityProfile:
        ...
        return self._security_profiles.get(
            profile_id, self._security_profiles["racing_assistant"]
        )
    ...
    def health_check(self) -> Dict:
        ...
        return {
            ...
            "rate_limited_users": len(
                [k for k, v in self._rate_limits.items() if v.tokens < 1]
            ),
        }
