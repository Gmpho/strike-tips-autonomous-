...
class RacingService:
    ...
    async def get_intelligence(self, track: str, intelligence_type: str = "Computaform SA", specific_date: Optional[str] = None) -> Dict[str, Any]:
        """Proactively harvest and parse intelligence."""
        # Warm cache first
        await self.harvester.get_latest_racing_intelligence(track=track, intelligence_type=intelligence_type, specific_date=specific_date)
        # Fetch/Return
        return await self.harvester.get_latest_racing_intelligence(track=track, intelligence_type=intelligence_type, specific_date=specific_date)

    async def scan_and_analyze(self, track: str, date_str: Optional[str] = None) -> List[Dict]:
        ...
        try:
            # Pre-warm
            await self.harvester.get_latest_racing_intelligence(track=track, intelligence_type="Computaform SA", specific_date=date_str)
            ...
            for i, ai_res in enumerate(ai_results):
                ...
                results.append({
                    "track": race.track, "race_number": race.race_number,
                    "race_time": race.race_time, "distance": race.distance,
                    "condition": race.track_condition, "runners": len(race.runners),
                    "value_bets": [asdict(vb) for vb in bets],
                    "ai_insight": ai_res.content[:500] + "..." if ai_res.content else "No insight"
                })
    ...
    def _convert_race_data(self, scraped_race: ScrapedRace) -> tuple:
        ...
        for sr in scraped_race.runners:
            ...
            runners.append(Runner(
                ...
                weight=sr.weight, last_5_runs=form
            ))
        ...
        return RaceCard(
            ...
            track_condition=scraped_race.track_condition, runners=runners
        ), probs, reasoning
