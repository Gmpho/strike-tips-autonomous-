...
class OddscheckerScraper:
    ...
    def __init__(self):
        ...
        self.schema = {
            ...
            "fields": [
                {
                    "name": "horse_name",
                    "selector": "a.runner-name, .horse-name, .name",
                    "type": "text",
                },
                {
                    "name": "fractional_odds",
                    "selector": "span.odds, .odds-value, .price",
                    "type": "text",
                },
                {
                    "name": "race_name",
                    "selector": "div.race-details h3, .event-name",
                    "type": "text",
                },
            ],
        }
    ...
    async def get_latest_odds(self) -> Dict:
        ...
        try:
            ...
            logger.warning(
                f"[OddsMonitor] Oddschecker scrape failed ({e}). Failing over to Betway API..."
            )
    ...
    async def _scrape_oddschecker(self) -> Dict:
        ...
        async with AsyncWebCrawler(config=self.browser_conf) as crawler:
            ...
            if not fusion_map:
                logger.warning(
                    "⚠️ Crawl4AI found 0 races. Page structure may have changed."
                )
                raise Exception("Found 0 races on Oddschecker")

            logger.info(
                f"✅ Crawl4AI Success: Found {len(fusion_map)} races on Oddschecker."
            )
            return fusion_map
