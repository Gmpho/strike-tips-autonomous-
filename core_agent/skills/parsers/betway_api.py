            ...
            for attempt in range(3):
                try:
                    ...
                    for reg in data.get("regions", []):
                        ...
                        for e in reg.get("sportEvents", []):
                            if not e.get("isFinished", True):
                                event_ids.append(
                                    (e["eventId"], reg.get("name"), e.get("league"))
                                )
                    ...
                    for eid, reg_name, league in event_ids:
                        tasks.append(
                            self._fetch_event_safe(client, eid, reg_name, league)
                        )
    ...
    async def get_snapshot_format(self) -> Dict[str, Any]:
        ...
        for det in raw.get("details", []):
            try:
                ...
                price_map = {
                    str(p.get("outcomeId")): p.get("priceDecimal", 5.0)
                ...
                for outcome in res.get("outcomes", []):
                    horse_name = (
                        outcome.get("outcomeName") or outcome.get("name") or "Unknown"
                    )
                ...
                if start_epoch:
                    try:
                        race_time = datetime.fromtimestamp(start_epoch).strftime(
                            "%H:%M"
                        )
                    except:
                        pass
                ...
                if race_num == 0:
                    num_match = re.search(
                        r"Race\s+(\d+)", det.get("name", ""), re.IGNORECASE
                    )
                    if num_match:
                ...
                logger.error(
                    f"Error parsing event detail for {det.get('eventId', 'Unknown')}: {e}"
                )
