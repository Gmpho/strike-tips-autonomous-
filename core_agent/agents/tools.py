...
def build_tools(strike):
    ...
    def calculate_probability_edge(
        ...
        estimated_probability: Annotated[
            float, Field(description="Your estimated win probability 0-1")
        ],
    ) -> dict:
    ...
    def calculate_max_position(
        ...
        return TOOL_REGISTRY["calculate_max_position"](
            edge_percent=edge_percent, strike=strike
        )
    ...
    def search_past_races(
        query: Annotated[
            str, Field(description="Search query e.g. 'Turffontein 1600m form'")
        ],
        n_results: Annotated[
            int, Field(description="Number of results", default=5)
        ] = 5,
    ) -> dict:
        """Semantic search over historical race data in ChromaDB memory."""
        return TOOL_REGISTRY["search_past_races"](
            query=query, n_results=n_results, strike=strike
        )
    ...
    async def evaluate_race(
        ...
        return await TOOL_REGISTRY["evaluate_race"](
            track=track, race_number=race_number, strike=strike
        )
    ...
    async def run_daily_analysis(
        tracks: Annotated[
            Optional[list[str]], Field(description="List of tracks or None for all")
        ] = None,
    ) -> dict:
    ...
    def record_selection(
        ...
        confidence: Annotated[
            str, Field(description="VALUE | STRONG_VALUE | MARGINAL")
        ],
    ) -> dict:
